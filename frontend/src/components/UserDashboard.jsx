...
const ActivityItem = ({ type, amount, status, date, counterparty, counterpartyPic }) => {
  ...
  const typeLabel = { deposit: 'Deposit', withdrawal: 'Withdrawal', buy: 'P2P Buy', sell: 'P2P Sell' }[type] || type;
  const typeIcon = { deposit: '⬇️', withdrawal: '⬆️', buy: '🤝', sell: '💱' }[type] || '💸';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', paddingBottom: '40px', opacity: animIn ? 1 : 0, transform: animIn ? 'translateY(0)' : 'translateY(16px)', transition: 'all 0.4s ease' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px' }}>
        <StatCard icon="💰" label="Total Balance" value={`$${fmt(balance)}`} sub="Your USDT wallet" color="var(--gold)" />
        <StatCard icon="📤" label="Total Sent" value={`$${fmt(totalVolume || 0)}`} sub="All time" color="var(--danger)" />
        <StatCard icon="📥" label="Total Received" value={`$${fmt(totalDeposited || 0)}`} sub="All time" color="var(--teal)" />
